 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (unsigned int t = 0; t < N - 1; t++) {
      ...
      fg[0] += 6000*CppAD::pow(vars[a_start + t], 2);
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  double x    = state[X_IDX];
  double y    = state[Y_IDX];
  double psi  = state[PSI_IDX];
  double v    = state[V_IDX];
  double cte  = state[CTE_IDX];
  double epsi = state[EPSI_IDX];
  ...
  //
  // 4 * 10 + 2 * 9
  size_t n_vars = STATE_CNT * N + ACTUT_CNT * (N - 1U);
